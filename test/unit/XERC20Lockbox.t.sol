...
abstract contract Base is Test {
    ...
    function _deployLockbox(bool _isNative) internal returns (XERC20Lockbox _retLockbox) {
        XERC20Lockbox lockboxImpl = new XERC20Lockbox();
        bytes memory initData = abi.encodeCall(XERC20Lockbox.initialize, (address(_xerc20), address(_erc20), _isNative));
        TransparentUpgradeableProxy proxy =
            new TransparentUpgradeableProxy(address(lockboxImpl), address(this), initData);

        _retLockbox = XERC20Lockbox(payable(address(proxy)));
    }

    function setUp() public virtual {
        vm.startPrank(_owner);
        _nativeLockbox = _deployLockbox(true);
        _lockbox = _deployLockbox(false);
        vm.stopPrank();
    }
}
...
contract UnitWithdraw is Base {
    function testWithdraw(uint256 _amount) public {
        ...
        vm.mockCall(
            address(_erc20), abi.encodeWithSelector(IERC20.transfer.selector, _owner, _amount), abi.encode(true)
        );
